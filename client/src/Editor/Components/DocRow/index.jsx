...
const DowRow = ({ id, fileName, date }) => {
  ...
  const { user } = useContext(AuthContext);
    ...
    if (fileName != undefined)
      isCollaborativeEditing = fileName.includes("(CE)");
...
{/* <Link to={`/Editor(CE)/${id}/${fileName}`} className="flex items-center w-full"></Link> */}
  ...
  return (
    <>
    { fileName!==undefined ? (      <div className="flex max-w-3xl mx-auto items-center p-4 m-2  rounded-lg hover:bg-gray-300 cursor-pointer text-gray-700 text-sm bg-red-400">
        <Link
          to={
